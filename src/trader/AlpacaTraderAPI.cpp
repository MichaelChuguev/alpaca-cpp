...
std::vector<OptionContract> AlpacaTraderAPI::get_option_contracts(const std::vector<std::string>& underlying_symbols, const bool& show_deliverables, const OptionContractStatus& status, const DateTime& expiration_date, const DateTime& expiration_date_gte, const DateTime& expiration_date_lte, const std::string& root_symbol, const OptionContractType& type, const OptionContractStyle& style, const double& strike_price_gte, const double& strike_price_lte, const std::string& page_token, const int& limit, const bool& ppind) {
    auto qb = QueryBuilder("/v2/options/contracts")
        ...
        .add("status", status == ACTIVE ? "active" : "inactive")
        .add("root_symbol", root_symbol)
        .add("type", type == CALL ? "call" : "put")
        .add("style", style == AMERICAN ? "american" : "european")
...
static std::string subtype_to_string(Subtype s) {
    switch (s) {
        ...
        case FLOATING_RATE: return "floating_rate";
        default:            return "";
    }
}

static std::string bond_status_to_string(BondStatus s) {
    switch (s) {
        case OUTSTANDING:   return "outstanding";
        case MATURED:       return "matured";
        case PRE_ISSUANCE:  return "pre_issuance";
        default:            return "";
    }
}
...
std::vector<OrderResponse> AlpacaTraderAPI::get_orders(const OrderStatus& status, const int& limit, const DateTime& after, const DateTime& until, const Sort& direction, const bool& nested, const std::vector<std::string>& symbols, const OrderSide& side) {
    ...
    if (status == OPEN)   status_str = "open";
    else if (status == CLOSED) status_str = "closed";
    else if (status == ALL)    status_str = "all";
...
OrderResponse AlpacaTraderAPI::replace_order(const std::string& order_id, const int& qty, const OrderTimeInForce& time_in_force, const double& limit_price, const double& stop_price, const double& trail, const std::string& client_order_id) {
    json body;
    body["qty"] = qty;
    body["time_in_force"] = time_in_force_to_string(time_in_force);
    body["limit_price"] = limit_price;
    body["stop_price"] = stop_price;
    body["trail_price"] = trail;
...
static std::string intraday_reporting_to_string(IntradayReporting r) {
    switch (r) {
        case MARKET_HOURS:   return "market_hours";
        case EXTENDED_HOURS: return "extended_hours";
...
static std::string pnl_reset_to_string(PnlReset r) {
    return r == PER_DAY ? "per_day" : "no_reset";
}
