...
struct AccountActivity : public BaseActivity {
    ...
    DateTime date;
    double net_amount;
    double per_share_amount; // Only for dividend activities
    ...
    AccountActivity(const json& j) {
        ...
        if (j.contains("order_id") && j.contains("order_status") && j.contains("type") &&
            ...
            cum_qty = std::stod(j["cum_qty"].get<std::string>());
            leaves_qty = std::stod(j["leaves_qty"].get<std::string>());
            ...
            net_amount = std::stod(j["net_amount"].get<std::string>());
            if (j.contains("per_share_amount")) per_share_amount = std::stod(j["per_share_amount"].get<std::string>());
    ...
    json to_json() const {
        ...
        if (account_activity_type == "trade_activity") {
            ...
            j["per_share_amount"] = per_share_amount;
