...
TEST(TreasuryTest, ParseFromJson) {
    ...
    EXPECT_DOUBLE_EQ(treasury.close_price, 99.5);
    EXPECT_DOUBLE_EQ(treasury.close_yield_to_maturity, 4.42);
    EXPECT_DOUBLE_EQ(treasury.close_yield_to_worst, 4.42);
    EXPECT_DOUBLE_EQ(treasury.coupon, 4.375);
...
TEST(TreasuryTest, ToJsonRoundTrip) {
    ...
    EXPECT_DOUBLE_EQ(treasury2.coupon, treasury.coupon);
}
