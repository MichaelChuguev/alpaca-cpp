...
TEST(OrderResponseTest, ParseFromJson) {
    ...
    EXPECT_DOUBLE_EQ(order.qty.to_double(), 1.0);
...
TEST(OrderTest, ToJson) {
    Order order("AAPL", Decimal(1.0), OrderSide::BUY, OrderType::MARKET, OrderTimeInForce::DAY);
    json result = order.to_json();
...
TEST(LegTest, ToJson) {
    Leg leg("AAPL", OrderSide::BUY, Decimal(1.0), PositionIntent::BUY_TO_OPEN);
