...
TEST(AccountActivityTest, ParseTradeActivity) {
    ...
    EXPECT_EQ(act.qty, Decimal(10.0));
    EXPECT_EQ(act.cum_qty, Decimal(10.0));
    EXPECT_EQ(act.leaves_qty, Decimal(0.0));
    EXPECT_EQ(act.price, Decimal(150.25));
...
TEST(AccountActivityTest, ParseNonTradeActivity) {
    ...
    EXPECT_EQ(act.symbol, "AAPL");
    EXPECT_EQ(act.qty, Decimal(100.0));
    EXPECT_EQ(act.activity_sub_type, "DIV");
    EXPECT_EQ(act.net_amount, Decimal(24.50));
    EXPECT_EQ(act.per_share_amount, Decimal(0.245));
