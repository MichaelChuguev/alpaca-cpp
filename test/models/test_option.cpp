...
TEST(DeliverableTest, ParseFromJson) {
    ...
    EXPECT_DOUBLE_EQ(deliv.amount, 100.0);
    EXPECT_DOUBLE_EQ(deliv.allocation_percentage, 100.0);
...
TEST(OptionContractTest, ParseFromJson) {
    ...
    EXPECT_DOUBLE_EQ(opt.strike_price, 150.0);
    EXPECT_DOUBLE_EQ(opt.multiplier, 100.0);
    EXPECT_DOUBLE_EQ(opt.size, 100.0);
    EXPECT_DOUBLE_EQ(opt.open_interest, 5000.0);
    EXPECT_DOUBLE_EQ(opt.close_price, 5.25);
...
TEST(OptionContractTest, ParseWithMissingOptionalFields) {
    ...
    EXPECT_DOUBLE_EQ(opt.strike_price, 0.0);
}
