...
TEST(TraderAPIBodyTest, CreateOrderBody) {
    Order order("AAPL", 10, OrderSide::BUY, OrderType::MARKET, OrderTimeInForce::DAY);
...
TEST(TraderAPIBodyTest, CreateLimitOrderBody) {
    Order order("MSFT", 5, OrderSide::SELL, OrderType::LIMIT, OrderTimeInForce::GTC);
    order.limit_price = 400.0;
...
TEST(TraderAPIResponseTest, PositionArrayResponse) {
    ...
    EXPECT_DOUBLE_EQ(result[0].qty, 10.0);
}
...
TEST(TraderAPIResponseTest, CryptoWalletArrayResponse) {
    ...
    EXPECT_DOUBLE_EQ(result[0].qty, 1.5);
}
